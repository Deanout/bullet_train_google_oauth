class Ability
  ...
  def initialize(user)
    if user.present?
      ...
      if google_enabled?
        can [:read, :create, :destroy], Oauth::GoogleAccount, user_id: user.id
        can :manage, Integrations::GoogleInstallation, team_id: user.team_ids
        can :destroy, Integrations::GoogleInstallation, oauth_google_account: {user_id: user.id}
      end
